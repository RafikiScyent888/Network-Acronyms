# Network+ Acronym Arcade

A free, static website for studying the 135 acronyms from the **CompTIA Network+** exam objectives — as flashcards and four study games. No build step, no dependencies, no backend. Open `index.html` or host it on GitHub Pages.
...
All 135 acronym/expansion/definition entries were extracted from `Network Acronyms.pdf` (included in this repo), which organizes them into 18 topic categories (Networking Fundamentals, Routing & Switching, IP Addressing & DNS, Security Concepts, Wireless Networking, Cloud & Virtualization, and more). The category and "new to Net+" flag from the source PDF are preserved in the data file for future use, though the current games draw from the full combined pool. Data lives in [`data/acronyms.json`](data/acronyms.json) / [`js/acronyms-data.js`](js/acronyms-data.js).

## Project structure

```
├── index.html                 # Main tile dashboard (start here)
├── data/acronyms.json         # Source data (135 entries, with category + isNew fields)
├── js/
...
- Filter study sessions by category (e.g. only "Security Protocols" or "Routing & Switching") using the `category` field already in the data
- A toggle to study only the acronyms flagged `isNew` (new to Network+, not already covered in A+)
...
This project is provided as-is for personal, educational use. CompTIA and CompTIA Network+ are trademarks of CompTIA, Inc. This project is an independent study aid and is not affiliated with, endorsed by, or sponsored by CompTIA.
